...
Для совместимости между вкладками избранное хранится как общий снимок и отдельные записи по каждому идентификатору. При одновременном изменении разных записей вкладки перечитывают эти записи и сохраняют обе операции; старый общий формат остаётся читаемым и постепенно дополняется новым представлением.
